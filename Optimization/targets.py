...
ORIENT_TOLERANCE = 0.1  # Tolerance for the orientation preference, in radians
...
class TargetValue():
    ...
    def get_orient_cell_idx(orientation, data_store, sheet_name) -> list:
        ...
        idx_cells = [n for n, idx in enumerate(idxs) if circular_dist(orientation, orientations_cells[idx], numpy.pi) < ORIENT_TOLERANCE]
        return idx_cells
...
class OrientationTuningPreferenceTarget(OneBoundUpperTarget):
    def calculate_value(self, data_store):
        ...
        for seg in segments:
            ...
            if circular_dist(orientation, target_O, numpy.pi) < ORIENT_TOLERANCE:
                spike_rate_high = get_mean_rate(seg.spiketrains, idx_cells)
            elif circular_dist(orientation, orthogonal_O, numpy.pi) < ORIENT_TOLERANCE:
...
class OrientationTuningOrthoHighTarget(TargetValue):
    def calculate_value(self, data_store):
        ...
        for seg in segments:
            orientation = get_annotation(seg, "orientation")
            if circular_dist(orientation, orthogonal_O, numpy.pi) < ORIENT_TOLERANCE:
                spike_rate_ortho_high = get_mean_rate(seg.spiketrains, idx_cells)
                break
...
class ConductanceRatio(TargetValue):
    def calculate_value(self, data_store):
        """Compute ratio of Inhibitory to Excitatory conductance for spontaneous activity"""

        segments = self.filter_spont_stimulus(data_store, self.sheet_name)
        if not len(segments):
            return None
        
        esyn_ids = segments[0].get_stored_esyn_ids()
        isyn_ids = segments[0].get_stored_isyn_ids()

        esyn_mean = numpy.mean(numpy.array([numpy.array([numpy.mean(seg.get_esyn(idd)) for idd in esyn_ids]) for seg in segments]), axis=0)
        isyn_mean = numpy.mean(numpy.array([numpy.array([numpy.mean(seg.get_isyn(idd)) for idd in isyn_ids]) for seg in segments]), axis=0)
        # mean here is over the segments, not over the cells

        return isyn_mean.mean() / esyn_mean.mean()


class ModulationRatio(TargetValue):
    def calculate_value(self, data_store):
        """Compute ratio F1/F0 of the spiking rate signal for drifting grating stimulus
        
        This is a measure of how much complex/simple cells are
        """

        psth_bin_size = 10.0  # in ms

        segments = param_filter_query(data_store, sheet_name=self.sheet_name, st_name='FullfieldDriftingSinusoidalGrating', st_contrast=[100]).get_segments()
        if not len(segments):
            return None

        spiketrains = []

        # There are two orientations for the stimulus, so we use cells that are close to any of the two orientations
        for seg in segments:
            # 1. identify the co-oriented cells
            st_orientation = get_annotation(seg, "orientation")
            ...
            idx_cells = [n for n, idx in enumerate(idxs) if (circular_dist(st_orientation, orientations_cells[idx], numpy.pi) < 0.1)]

            # 2. Filter spiketrains accordingly
            full_spiketrains = seg.get_spiketrains()
            spiketrains.extend([full_spiketrains[idx] for idx in idx_cells])

        # 3. PSTH
        frequency = get_annotation(seg, "temporal_frequency")  # is Hz
        t_start = round(spiketrains[0].t_start, 5)
        t_stop = round(spiketrains[0].t_stop, 5)
        num_bins = int(round((t_stop - t_start) / psth_bin_size))
        r = (float(t_start), float(t_stop))
        psths = numpy.array([numpy.histogram(x, bins=num_bins, range=r)[0] / (psth_bin_size/1000)  for x in spiketrains])
        # (neuron, time_bin)

        # 4. FFT
        duration = t_stop - t_start  # in ms
        period = 1/frequency * 1000  # in ms
        cycles = duration / period
        first_har = int(round(float(cycles.magnitude)))

        fft = numpy.fft.fft(psths, axis=1)
        fft_abs = numpy.abs(fft).mean(axis=0)

        mod_ratio = 2*fft_abs[first_har]/fft_abs[0]
        return mod_ratio
